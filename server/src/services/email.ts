...
function template(booking: BookingEmailData): string {
  return `
<!DOCTYPE html>
<html>
...
export async function sendBookingConfirmation(to: string, booking: BookingEmailData): Promise<void> {
  const transporter = getTransporter();
  if (!to || !transporter) {
    console.log(`[EMAIL] Skipped: Booking #${booking.id} → ${to || 'no email'}`);
    return;
  }
  try {
