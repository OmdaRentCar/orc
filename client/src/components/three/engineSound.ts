...
let harmonics: { osc: OscillatorNode; gain: GainNode; baseFreq: number }[] = [];
...
export function playStartupSound(onComplete?: () => void): void {
  if (!audioCtx) initAudio();
  if (!audioCtx || !masterGain || started) return;
  started = true;
...
function startIdle(): void {
  ...
  harmonics = [150, 220, 310, 420].map((freq, i) => {
...
export function setRPM(frac: number): void {
  if (!audioCtx || !idleOsc || !idleGain) return;
  const now = audioCtx.currentTime;
  idleOsc.frequency.setTargetAtTime(75 + frac * 350, now, 0.1);
  idleGain.gain.setTargetAtTime(0.08 + frac * 0.25, now, 0.1);
  harmonics.forEach((h, i) => {
    h.osc.frequency.setTargetAtTime(h.baseFreq + frac * 600, now, 0.1);
    h.gain.gain.setTargetAtTime(0.03 / (i + 1) + frac * 0.08, now, 0.1);
  });
  if (noiseGain) noiseGain.gain.setTargetAtTime(0.02 + frac * 0.06, now, 0.1);
}
...
export function stopEngine(): void {
  ...
  if (masterGain) masterGain.gain.linearRampToValueAtTime(0, now + 0.3);
  started = false;
}
